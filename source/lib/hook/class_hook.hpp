...
#define INJECT_CLASS_HOOK(funcname, name)   \
    exl::hook::nx64::HookFunc(&_HOOK_CLASS_FUNC(name, funcname), &_HOOK_CLASS_HOOK_NAME(name));
#define INJECT_CLASS_HOOK_T(funcname, name) \
    _HOOK_CLASS_T_NAME(name) = exl::hook::nx64::HookFunc(&_HOOK_CLASS_FUNC(name, funcname), &_HOOK_CLASS_HOOK_NAME(name), true);
